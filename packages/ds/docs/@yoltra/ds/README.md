![Yoltra logo](https://yoltra.dev/assets/yoltra-logo.png)

[**@yoltra/ds**](../../README.md)

***

[@yoltra/ds](../../README.md) / @yoltra/ds

# @yoltra/ds

@yoltra/ds — Yoltra Design System.

Foundation tokens, semantic themes, a CSS-variable stylesheet generator, and
server-safe primitive React components shared across the Yoltra website,
docs, and examples. Theming is driven entirely by a `data-theme` attribute on
the document root, so these primitives render on the server.

Interactive controls (theme controller, tabs, copy button) that need React
state or browser APIs are published from the `@yoltra/ds/client`
entry (`@yoltra/ds/client`), which ships a real `"use client"` directive.

## Interfaces

- [BadgeProps](interfaces/BadgeProps.md)
- [ButtonGroupProps](interfaces/ButtonGroupProps.md)
- [ButtonLinkProps](interfaces/ButtonLinkProps.md)
- [ButtonProps](interfaces/ButtonProps.md)
- [CalloutProps](interfaces/CalloutProps.md)
- [CardProps](interfaces/CardProps.md)
- [ColorScale](interfaces/ColorScale.md)
- [ContainerProps](interfaces/ContainerProps.md)
- [DividerProps](interfaces/DividerProps.md)
- [EmptyStateProps](interfaces/EmptyStateProps.md)
- [FieldControlProps](interfaces/FieldControlProps.md)
- [FieldsetProps](interfaces/FieldsetProps.md)
- [FlowProps](interfaces/FlowProps.md)
- [FontStyleToken](interfaces/FontStyleToken.md)
- [FontTokens](interfaces/FontTokens.md)
- [FormFieldProps](interfaces/FormFieldProps.md)
- [FoundationTokens](interfaces/FoundationTokens.md)
- [GridProps](interfaces/GridProps.md)
- [HeadingProps](interfaces/HeadingProps.md)
- [IconButtonProps](interfaces/IconButtonProps.md)
- [InlineProps](interfaces/InlineProps.md)
- [InputProps](interfaces/InputProps.md)
- [LinkProps](interfaces/LinkProps.md)
- [PaletteTokens](interfaces/PaletteTokens.md)
- [RadioGroupProps](interfaces/RadioGroupProps.md)
- [SelectProps](interfaces/SelectProps.md)
- [SemanticColorTokens](interfaces/SemanticColorTokens.md)
- [SkeletonProps](interfaces/SkeletonProps.md)
- [SliderProps](interfaces/SliderProps.md)
- [SpinnerProps](interfaces/SpinnerProps.md)
- [TextareaProps](interfaces/TextareaProps.md)
- [TextProps](interfaces/TextProps.md)
- [ThemeTokens](interfaces/ThemeTokens.md)
- [ToggleProps](interfaces/ToggleProps.md)
- [VisuallyHiddenProps](interfaces/VisuallyHiddenProps.md)

## Type Aliases

- [Align](type-aliases/Align.md)
- [ButtonSize](type-aliases/ButtonSize.md)
- [ButtonVariant](type-aliases/ButtonVariant.md)
- [CalloutKind](type-aliases/CalloutKind.md)
- [CardElevation](type-aliases/CardElevation.md)
- [CheckboxProps](type-aliases/CheckboxProps.md)
- [CSSLength](type-aliases/CSSLength.md)
- [FieldSize](type-aliases/FieldSize.md)
- [Justify](type-aliases/Justify.md)
- [RadioProps](type-aliases/RadioProps.md)
- [SpaceToken](type-aliases/SpaceToken.md)
- [StackProps](type-aliases/StackProps.md)
- [SwitchProps](type-aliases/SwitchProps.md)
- [TextSize](type-aliases/TextSize.md)
- [TextTone](type-aliases/TextTone.md)
- [ThemeId](type-aliases/ThemeId.md)

## Variables

- [darkTheme](variables/darkTheme.md)
- [foundationTokens](variables/foundationTokens.md)
- [lightTheme](variables/lightTheme.md)
- [themes](variables/themes.md)

## Functions

- [Badge](functions/Badge.md)
- [Button](functions/Button.md)
- [ButtonGroup](functions/ButtonGroup.md)
- [ButtonLink](functions/ButtonLink.md)
- [Callout](functions/Callout.md)
- [Card](functions/Card.md)
- [Checkbox](functions/Checkbox.md)
- [Container](functions/Container.md)
- [Divider](functions/Divider.md)
- [EmptyState](functions/EmptyState.md)
- [Fieldset](functions/Fieldset.md)
- [FormField](functions/FormField.md)
- [Grid](functions/Grid.md)
- [Heading](functions/Heading.md)
- [IconButton](functions/IconButton.md)
- [Inline](functions/Inline.md)
- [InlineCode](functions/InlineCode.md)
- [Input](functions/Input.md)
- [Kbd](functions/Kbd.md)
- [Link](functions/Link.md)
- [Radio](functions/Radio.md)
- [RadioGroup](functions/RadioGroup.md)
- [Select](functions/Select.md)
- [Skeleton](functions/Skeleton.md)
- [Slider](functions/Slider.md)
- [Spinner](functions/Spinner.md)
- [Stack](functions/Stack.md)
- [Switch](functions/Switch.md)
- [Table](functions/Table.md)
- [TBody](functions/TBody.md)
- [TD](functions/TD.md)
- [Text](functions/Text.md)
- [Textarea](functions/Textarea.md)
- [TH](functions/TH.md)
- [THead](functions/THead.md)
- [themeCss](functions/themeCss.md)
- [TR](functions/TR.md)
- [VisuallyHidden](functions/VisuallyHidden.md)
